...
def list_dirs_classes(directory, classes):
    ...
    """
    return [pathlib.Path(directory + '/' + c) for c in classes if pathlib.Path(directory + '/' + c).is_dir()]
...
def ratio(input, output="output", seed=1337, ratio=(0.8, 0.1, 0.1), classes=None):
